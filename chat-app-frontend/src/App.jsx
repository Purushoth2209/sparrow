...
const PrivateRoute = ({ element: Component, ...rest }) => {
  ...
  React.useEffect(() => {
    const checkAuth = async () => {
      ...
      console.log('🔍 Auth check - Local storage:', { token: !!token, profileId: !!profileId });
...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    const handleBeforeUnload = (event) => {
      ...
      // Use synchronous XMLHttpRequest for better reliability on page unload
      const profileId = localStorage.getItem('profileId');
      if (profileId && window.socketInstance) {
        try {
          // Send logout notification via Socket.IO synchronously
      ...
      // Send synchronous logout request to backend
      if (profileId) {
        try {
          const xhr = new XMLHttpRequest();
          xhr.open('POST', 'http://localhost:5000/api/auth/logout', false); // Synchronous
          xhr.setRequestHeader('Content-Type', 'application/json');
          xhr.withCredentials = true;
          xhr.send(JSON.stringify({}));
          console.log('📡 Synchronous logout request sent');
        } catch (error) {
          console.error('Error sending synchronous logout:', error);
        }
      }
