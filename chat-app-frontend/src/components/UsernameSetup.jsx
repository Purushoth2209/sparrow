...
const UsernameSetup = () => {
  ...
  const handleUsernameChange = async (e) => {
    ...
    if (value.length >= 3) {
      try {
        const response = await axios.get(`http://localhost:5000/api/auth/check-username?username=${encodeURIComponent(value)}`, {
          withCredentials: true
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/set-username', {
